...
% Dimesnions of matrices are 25*4*11 where 25 stands for number of signals,
% 4 stands for number of channels inside every signal, and 11 stands for 
% number of 10s segments with 5s overlapping
...
for nm = 1:25
    ...
    loc = ['D:\Fakultet\Master\Master rad\set-a-text\',name,'.csv'];
    data = readmatrix(loc);
    mQRS_annot = load(['D:\Fakultet\phD\Rad za casopis\Kodovi\mQRS_annot\mQRS_',name,'_ch1.mat']);
    mQRS_annot1 = mQRS_annot.locs1;
    nm_limit = 1;
    
% every 10s of signal is assessed with 5s widnow overlap
    for limit1 = (1:5000:55000) 
        
        limit2 = limit1 + 9999;
        annot = load(['D:\Fakultet\Master\Master rad\set-a-text\',name,'.fqrs.txt']);
